...
RegisterNetEvent("Lasers:Server:Save", function(laser)
...
function parseLaser(laser)
	...
	out = out
		.. string.format(
			"travelTimeBetweenTargets = {%s, %s}, ",
			tostring(round(laser.travelTimeBetweenTargets[1], 3)),
			tostring(round(laser.travelTimeBetweenTargets[2], 3))
		)
	out = out
		.. string.format(
			"waitTimeAtTargets = {%s, %s}",
			tostring(round(laser.waitTimeAtTargets[1], 3)),
			tostring(round(laser.waitTimeAtTargets[2], 3))
		)
	if #laser.originPoints == 1 then
