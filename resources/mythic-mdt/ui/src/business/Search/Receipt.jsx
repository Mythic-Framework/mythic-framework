...
export default () => {
	...
	const onCreate = () => {
		history(`/business/create/receipt`);
	};
	...
	const confirmDeleteAll = async () => {
		...
		try {
            ...
            if (res) {
                toast.success('Cleared Receipts');
				fetch(searchTerm);
            } else {
                toast.error('Unable to Clear Receipts');
            };
			setLoading(false);
        } catch (err) {
            console.log(err);
			toast.success('Unable to Clear Receipts');
			setLoading(false);
        }
	};

	const canClearReceipts = hasPermission('TABLET_CLEAR_RECEIPT', false);
	...
	return (
		<div className={classes.wrapper}>
			...
			<DeleteConfirmation open={clearingModal} onCancel={() => setClearingModal(false)} onConfirm={() => confirmDeleteAll()} />
		</div>
	);
};
