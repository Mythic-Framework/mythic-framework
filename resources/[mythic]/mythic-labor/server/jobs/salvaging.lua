...
AddEventHandler("Labor:Server:Startup", function()
	...
	Callbacks:RegisterServerCallback("Salvaging:SalvageCar", function(source, data, cb)
		...
		local entity = NetworkGetEntityFromNetworkId(data)
		...
		if char:GetData("TempJob") ~= _JOB or not _joiners[source] or not _salvaging[_joiners[source]] then
			cb(false)
			return
		end

		local job = _salvaging[_joiners[source]]

		if job.entities[data] == nil then
			job.entities[data] = false
		end

		if job.entities[data] then
			cb(false)
			return
		end

		job.entities[data] = true

		local randomLoot = Utils:WeightedRandom(_lootTable)
		local randomLootAmount = math.random(randomLoot.min, randomLoot.max)
		Inventory:AddItem(char:GetData("SID"), randomLoot.item, randomLootAmount, {}, 1)

		local salvagedPartsAmount = math.random(10)
		Inventory:AddItem(char:GetData("SID"), "salvagedparts", salvagedPartsAmount, {}, 1)
