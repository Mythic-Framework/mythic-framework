...
function Laser.new(originPoint, targetPoints, options)
	...
	if options.color then
		r, g, b, a = table.unpack(options.color)
	end
	local extensionEnabled = true
	if options.extensionEnabled ~= nil then
		extensionEnabled = options.extensionEnabled
	end
	local randomTargetSelection = true
	if options.randomTargetSelection ~= nil then
		randomTargetSelection = options.randomTargetSelection
	end
	...
	function self._startMultiOriginLaser()
		...
		assert(
			#originPoint > 1 and #targetPoints > 1,
		...
		CreateThread(function()
			...
			while active do
				...
				local currentTargetPoint =
					calculateCurrentPoint(fromTargetPoint, toTargetPoint, deltaTime, currentTravelTime)
				local fromOriginPoint = originPoint[fromIndex]
				local toOriginPoint = originPoint[toIndex]
				local currentOriginPoint =
					calculateCurrentPoint(fromOriginPoint, toOriginPoint, deltaTime, currentTravelTime)
